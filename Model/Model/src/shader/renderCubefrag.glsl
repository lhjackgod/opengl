...
vec3 calPointLight(pointLight plight, vec3 n, vec3 cameraDir) {
	...
	vec3 diffuseColor1 = max(0.0, dot(n, lightDir)) * plight.diffuse * vec3(texture(texture_diffuse1, outUV)) * attenuation;
	vec3 diffuseColor2 = max(0.0, dot(n, lightDir)) * plight.diffuse * vec3(texture(texture_diffuse2, outUV)) * attenuation;
	vec3 diffuseColor3 = max(0.0, dot(n, lightDir)) * plight.diffuse * vec3(texture(texture_diffuse3, outUV)) * attenuation;
	vec3 h = normalize(lightDir + cameraDir);
	vec3 specular1 = plight.specular * vec3(texture(texture_specular1, outUV)) * pow(max(0.0, dot(n, h)), material.k) * attenuation;
	vec3 specular2 = plight.specular * vec3(texture(texture_specular2, outUV)) * pow(max(0.0, dot(n, h)), material.k) * attenuation;
	return specular1 + specular2 + diffuseColor1 + diffuseColor2 + diffuseColor3;
}
...
void main() {
	...
	vec3 resultColor = resultPointColor + ambientColor;
	FragColor = vec4(resultPointColor, 1.0);
}